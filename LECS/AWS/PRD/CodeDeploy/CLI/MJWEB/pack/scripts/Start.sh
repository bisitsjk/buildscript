...
MJ_T_ARN="arn:aws:elasticloadbalancing:ap-northeast-2:313526127569:targetgroup/LECS-PRD-MJ-WEB-80/b9d08596aa573f46"
...
do
	...
	aws autoscaling attach-load-balancer-target-groups --auto-scaling-group-name ${ASG_NAME} --target-group-arns ${MJ_T_ARN} --region $awsDefaultRegion
    sleep 5
    ATACH_CHECK_TARGET=$(aws autoscaling describe-auto-scaling-groups --auto-scaling-group-names $ASG_NAME --query 'AutoScalingGroups[].TargetGroupARNs[]' --region $awsDefaultRegion --output text)
done
...
do
	...
	aws autoscaling detach-load-balancer-target-groups --auto-scaling-group-name ${OLD_ASG_NAME} --target-group-arns ${MJ_T_ARN} --region $awsDefaultRegion
    sleep 5
    DETACH_CHECK_TARGET=$(aws autoscaling describe-auto-scaling-groups --auto-scaling-group-names ${OLD_ASG_NAME} --query 'AutoScalingGroups[].TargetGroupARNs[]'  --region $awsDefaultRegion --output text)
done
