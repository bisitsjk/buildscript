...
echo "Remove web 1set ..."

aws elbv2 deregister-targets --target-group-arn $BOWEB_T_ARN --targets Id=$BO1_EC2_ID --region $awsDefaultRegion

sleep 1

aws elbv2 deregister-targets --target-group-arn $CCWEB_T_ARN --targets Id=$CC1_EC2_ID --region $awsDefaultRegion

sleep 1

aws elbv2 deregister-targets --target-group-arn $POWEB_T_ARN --targets Id=$PO1_EC2_ID --region $awsDefaultRegion
