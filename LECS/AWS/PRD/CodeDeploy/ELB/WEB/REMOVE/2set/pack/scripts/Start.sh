...
echo "Remove web 2set ..."

aws elbv2 deregister-targets --target-group-arn $BOWEB_T_ARN --targets Id=$BO2_EC2_ID --region $awsDefaultRegion

sleep 1

aws elbv2 deregister-targets --target-group-arn $CCWEB_T_ARN --targets Id=$CC2_EC2_ID --region $awsDefaultRegion

sleep 1

aws elbv2 deregister-targets --target-group-arn $POWEB_T_ARN --targets Id=$PO2_EC2_ID --region $awsDefaultRegion

sleep 1

until [ "$status" = unused ]
	do
		status=$(aws elbv2 describe-target-health --targets Id=$BO2_EC2_ID,Port=80 --target-group-arn $BOWEB_T_ARN --region $awsDefaultRegion | grep State | cut -d\" -f4);
...
until [ "$status" = unused ]
	do
		status=$(aws elbv2 describe-target-health --targets Id=$CC2_EC2_ID,Port=80 --target-group-arn $CCWEB_T_ARN --region $awsDefaultRegion | grep State | cut -d\" -f4);
		echo "checkstate ="$status
		sleep 10
	done
...
until [ "$status" = unused ]
	do
		status=$(aws elbv2 describe-target-health --targets Id=$PO2_EC2_ID,Port=80 --target-group-arn $POWEB_T_ARN --region $awsDefaultRegion | grep State | cut -d\" -f4);
		echo "checkstate ="$status
		sleep 10
	done
