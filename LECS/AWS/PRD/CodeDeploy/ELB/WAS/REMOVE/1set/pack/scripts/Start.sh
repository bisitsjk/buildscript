...
echo "REMOVE was 1set ..."

#aws elb register-instances-with-load-balancer --load-balancer-name $BOWAS_ELB_NAME --instances $BO1_EC2_ID --region $awsDefaultRegion
...
#aws elb register-instances-with-load-balancer --load-balancer-name $CCWAS_ELB_NAME --instances $CC1_EC2_ID --region $awsDefaultRegion
...
#aws elb register-instances-with-load-balancer --load-balancer-name $POWAS_ELB_NAME --instances $PO1_EC2_ID --region $awsDefaultRegion
...
until [ "$status" = OutOfService ]
	do
		status=$(aws elb describe-instance-health --load-balancer-name $BOWAS_ELB_NAME --instances $BO1_EC2_ID --region $awsDefaultRegion | grep State | cut -d\" -f4 | awk 'NR==2{print $1}');
...
until [ "$status" = OutOfService ]
	do
		status=$(aws elb describe-instance-health --load-balancer-name $CCWAS_ELB_NAME --instances $CC1_EC2_ID --region $awsDefaultRegion | grep State | cut -d\" -f4 | awk 'NR==2{print $1}');
		echo "checkstate = $status"
		sleep 5
	done
...
until [ "$status" = OutOfService ]
	do
		status=$(aws elb describe-instance-health --load-balancer-name $POWAS_ELB_NAME --instances $PO1_EC2_ID --region $awsDefaultRegion | grep State | cut -d\" -f4 | awk 'NR==2{print $1}');
		echo "checkstate = $status"
		sleep 5
	done
